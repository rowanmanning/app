...
const flexiBody = require('./middleware/flexi-body');
...
class App {
	...
	initModels() {
		try {
			const schemas = requireAll({
				dirname: this.paths.model,
				map: camelcase,
				resolve: initModel => initModel(this)
			});
			for (const [name, schema] of Object.entries(schemas)) {
				this.models[name] = this.db.model(name, schema);
				this.log.debug(`[setup/models]: "${name}" model initialised`);
			}
		} catch (error) {
	...
	initExpress() {
		...
		// Automatically parse the request body
		app.use(flexiBody());
		...
		if (this.options.sessionSecret) {
			app.use(session({
				...
				secret: this.options.sessionSecret || `${this.name} session secret`,
				store: (
	...
	initControllers() {
		try {
			const controllers = requireAll({
				dirname: this.paths.controller,
				map: camelcase,
				resolve: initController => initController(this)
			});
			for (const [name, controller] of Object.entries(controllers)) {
				this.controllers[name] = controller;
				this.log.debug(`[setup/controllers]: "${name}" controller initialised`);
			}
		} catch (error) {
